  ...
  return (
    <>
      <img
        src={first.src}
        alt=""
        className={`absolute left-1/2 top-11 origin-bottom -translate-x-[calc(50%+38px)] translate-y-1.5 -rotate-[7deg] ${tile} ${spring} group-hover:-translate-x-[calc(50%+92px)] group-hover:-translate-y-4 group-hover:-rotate-[15deg]`}
      />
      <img
        src={third.src}
        alt=""
      ...
      <img
        src={second.src}
        alt=""
        className={`absolute left-1/2 top-11 z-10 origin-bottom -translate-x-1/2 ${tile} ${spring} group-hover:-translate-y-12`}
      />
      {/* Иконки и звёздочки видны только при наведении (см. .case-card в index.css) */}
      <div className="pointer-events-none absolute inset-0 z-20" aria-hidden="true">
        {icons.map((icon, i) => (
...
function WebFanPreview({ web }: { web: CaseWebScreens }) {
  ...
  return (
    <>
      ...
      <BrowserMockup
        src={web.site.src}
        className={`absolute left-1/2 top-[72px] z-10 w-[56%] -translate-x-1/2 ${spring} group-hover:-translate-y-7`}
      />
