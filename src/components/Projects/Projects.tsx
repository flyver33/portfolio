...
function StockPreview({ kind }: { kind: NonNullable<CaseInfo['stockPreview']> }) {
  ...
  if (kind === 'browser') {
    return (
      <div className={`${screen} absolute inset-x-8 top-10 bottom-0 transition-transform duration-300 ease-out fan-open:-translate-y-3`}>
  ...
  if (kind === 'phone') {
    return (
      <div className={`${screen} absolute left-1/2 top-12 bottom-0 w-24 -translate-x-1/2 rounded-t-xl transition-transform duration-300 ease-out fan-open:-translate-y-4`}>
  ...
  return (
    <>
      <div className={`${screen} absolute left-6 top-14 bottom-0 w-28 -rotate-3 transition-transform duration-300 ease-out fan-open:-translate-y-3 fan-open:-rotate-6`} />
      <div className={`${screen} absolute right-6 top-9 bottom-0 w-28 rotate-2 transition-transform duration-300 ease-out fan-open:-translate-y-5 fan-open:rotate-4`} />
    </>
  )
}
...
function FanPreview({
  ...
  return (
    <>
      <PhoneMockup
        src={first.src}
        className={`absolute left-1/2 top-7 w-28 origin-bottom -translate-x-[calc(50%+34px)] translate-y-1.5 -rotate-[7deg] ${spring} fan-open:-translate-x-[calc(50%+80px)] fan-open:-translate-y-4 fan-open:-rotate-[15deg]`}
      />
      <PhoneMockup
        src={third.src}
        className={`absolute left-1/2 top-7 w-28 origin-bottom translate-x-[calc(-50%+34px)] translate-y-1.5 rotate-[7deg] ${spring} fan-open:translate-x-[calc(-50%+80px)] fan-open:-translate-y-4 fan-open:rotate-[15deg]`}
      ...
      <PhoneMockup
        src={second.src}
        className={`absolute left-1/2 top-7 z-10 w-28 origin-bottom -translate-x-1/2 ${spring} fan-open:-translate-y-10`}
      />
...
function WebFanPreview({ web }: { web: CaseWebScreens }) {
  ...
  return (
    <>
      <PhoneMockup
        src={left.src}
        className={`absolute left-[8%] top-14 w-[88px] origin-bottom -rotate-[8deg] ${spring} fan-open:-translate-x-4 fan-open:-translate-y-3 fan-open:-rotate-[14deg]`}
      />
      <PhoneMockup
        src={right.src}
        className={`absolute right-[8%] top-14 w-[88px] origin-bottom rotate-[8deg] ${spring} fan-open:translate-x-4 fan-open:-translate-y-3 fan-open:rotate-[14deg]`}
      />
      <BrowserMockup
        src={web.site.src}
      ...
      <div className="pointer-events-none absolute inset-0 z-20" aria-hidden="true">
        <BigStars />
        {/* Мелкие угловые — со сдвигом вниз от крупных BigStars, чтобы не накладывались */}
        <Star className="confetti absolute left-[2%] top-[2%] w-3 text-accent" style={fly(44, 170, 18, 60)} />
        <Star className="confetti absolute right-[1%] -top-3 w-4 text-accent" style={fly(-52, 186, -14, 150)} />
...
function SoloWebPreview({ screen }: { screen: CaseScreen }) {
  return (
    <>
      <BrowserMockup
        src={screen.src}
        className={`absolute left-1/2 top-10 w-[72%] -translate-x-1/2 ${spring} fan-open:-translate-y-6`}
      />
      {/* Звёздочки видны только при наведении (см. .case-card в index.css) */}
      <div className="pointer-events-none absolute inset-0 z-20" aria-hidden="true">
        <BigStars />
        {/* Мелкие угловые — со сдвигом вниз от крупных BigStars, чтобы не накладывались */}
        <Star className="confetti absolute left-[3%] top-[3%] w-3 text-accent" style={fly(42, 168, 20, 70)} />
        <Star className="confetti absolute right-[1%] -top-2 w-4 text-accent" style={fly(-50, 184, -16, 160)} />
...
function Projects() {
  const { active, register } = useActiveCard()
  return (
    <section id="projects" className="relative scroll-mt-8 overflow-x-clip pt-32">
          ...
          {CASES.map((item) => (
            <a
              ...
              className={`case-card group relative block rounded-2xl border border-line bg-elevated shadow-(--shadow-card) transition-all duration-300 ease-out hover:z-20 hover:-translate-y-2 hover:shadow-(--shadow-hover) ${
                active === item.slug ? 'is-active z-20 -translate-y-2 shadow-(--shadow-hover)' : ''
              }`}
            >
              <div
